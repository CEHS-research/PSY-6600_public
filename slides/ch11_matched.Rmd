...
- **Before-and-after** or .bluer[longitudinal] designs
    + Pre- / post-test
    + time 1 / time 2
...
- **Cross-over designs** each participant gets both conditions
    + Order effects? 
    + Need to .bluer[counterbalance] order
...
- .dcoral[**Counterbalancing**] random subset of cases 
    + Half get: A then B
    + Half get: B then A
    + May not eliminate .bluer[carry-over effects]
    + May need a .bluer[Wash-out period]
...
### Example) Data Wrangling - Create a "DIFFERENCE" score
...
### Example) Data Wrangling - Restructure to "LONG" format
...
### Example) Both Time points: Histograms & Boxplots
...
### Example) Both Time points: Line Plot and Scatter Plot
...
### Example) Change Scores: Histograms & Boxplots
...
### Example) Direct Difference Method (Ch 7. 1-sample test)
...
### Example) Paired t-Test Method (adjustment to the indept t-test)
...
### Example) One-Sided Alternative Tests and Confidence Intervals
...
### Example) Effect Size: Cohen's d for MATCHED samples
...
### Example) Effect Size: Hedge's g for MATCHED samples
