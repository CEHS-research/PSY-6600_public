...
data_wide <- data.frame(control_placebo   = c(24, 20, 29, 20, 28),
                        ...
                        interrup_placebo  = c(16, 20, 11, 19, 14),
...
data_long %>% 
  ...
  furniture::table1(score, digits = 2)
```
...
```{r}
aov_2way <- data_long %>% 
  afex::aov_4(score ~ sleep*stimulant + (1|id),
              data = .) 
```
...
```{r}
aov_2way %>% 
  emmeans::emmeans(~ sleep*stimulant)
```
...
aov_2way %>% 
  emmeans::emmeans(~ sleep*stimulant) %>% 
  emmeans::contrast(list("caffeine: cntrl vs. total deprivation" = 
                           c(1,  0,  0, -1, 
                            -1,  0,  0,  1,
                             0,  0,  0,  0)))
```
