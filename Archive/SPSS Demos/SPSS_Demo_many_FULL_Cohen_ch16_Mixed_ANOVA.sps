...
GLM Before After FollowUp BY Phobia
  /WSFACTOR=time 3  
  /PLOT=PROFILE(time*Phobia)
  /EMMEANS TABLES(time*Phobia) COMPARE(phobia)
  /EMMEANS TABLES(time*Phobia) COMPARE(time)
  /WSDESIGN=time 
  /DESIGN=Phobia.
...
GLM Happiness Fear BY Preload
  /WSFACTOR=movie_type 2  
  /WSDESIGN=movie_type 
  /PLOT=PROFILE(movie_type*Preload)
  /EMMEANS tables(movie_type*Preload) compare(movie_type)
  /EMMEANS tables(movie_type*Preload) compare(preload)
  /DESIGN=Preload.
