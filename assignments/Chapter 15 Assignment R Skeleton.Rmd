...
fit_memory <- memory_long %>% 
  afex::aov_4(recall ~ 1 + (stimuli|id),
              data = .)
...
fit_anx_female <- ihno_anx_long %>% 
  dplyr::filter(genderF == "Female") %>% 
  afex::aov_4(anxiety ~ 1 + (time|sub_num),
              data = .)

summary(fit_anx_female)
```
...
```{r} 
# RM ANOVA: Mauchle Tests for Sphericity with and without corrections applied
fit_anx_male <- ihno_anx_long %>% 
  dplyr::filter(genderF == "Male") %>% 
  afex::aov_4(anxiety ~ 1 + (time|sub_num),
              data = .)

summary(fit_anx_male)
```
...
ihno_anx_long %>% 
  dplyr::filter(genderF == "Female") %>% 
  dplyr::filter(time %in% c("baseline", "pre-quiz")) %>% 
  t.test(anxiety ~ time,
         data = .,
         paired = TRUE)
```
...
ihno_anx_long %>% 
  dplyr::filter(genderF == "Female") %>% 
  dplyr::filter(time %in% c("baseline", "post-quiz")) %>% 
  t.test(anxiety ~ time,
         data = .,
         paired = TRUE)
```
...
ihno_anx_long %>% 
  dplyr::filter(genderF == "Female") %>% 
  dplyr::filter(time %in% c("pre-quiz", "post-quiz")) %>% 
  t.test(anxiety ~ time,
         data = .,
         paired = TRUE)
```
